...
rankingData.players = [
  ...
  { title: "アシスト数ランキング", unit: "本", rows: [] },
  { title: "イエローカード数ランキング", unit: "枚", rows: [] },
  { title: "レッドカード数ランキング", unit: "枚", rows: [["Sphephelo Sithole", "RSA", 1], ["Themba Zwane", "RSA", 1], ["Cesar Montes", "MEX", 1]] }
];
  ...
  const grouped = rows.reduce((acc, row) => {
    ...
    if (type === "players") {
      acc[metric].rows.push({
        rank: numberValue(row.rank),
        value: [row.player_name || "", row.team_code || "", numberValue(row.value), numberValue(row.rank)]
      });
    } else {
      const code = row.team_code || "";
      ...
      acc[metric].rows.push({
        rank: numberValue(row.rank),
        value: [code, numberValue(row.value), numberValue(row.rank)]
      });
    }
    return acc;
  }, {});
...
function applySheetRankings(countryRows, playerRows, fifaRows) {
  ...
  if (nextPlayers.length) {
    rankingData.players = nextPlayers;
  }
  if (nextFifa.length) rankingData.fifa = nextFifa;
}

function goalEventsFromMatch(match) {
  const teamByName = new Map([
    [match.home?.name, match.home?.code],
    [match.away?.name, match.away?.code]
  ]);

  return (match.scorers || []).flatMap(line => {
    const [teamName, scorerText = ""] = String(line).split(":");
    const code = teamByName.get(teamName.trim());
    if (!code) return [];
    return scorerText
      .split(",")
      ...
      .flatMap(entry => {
        const countMatch = entry.match(/\s*x(\d+)$/i);
        const count = countMatch ? Number(countMatch[1]) : 1;
        const name = entry
          .replace(/\s*x\d+$/i, "")
          .replace(/^\d+(?:\+\d+)?'\s*/, "")
          .trim();
        if (!name || /^own goal$/i.test(name) || name === "オウンゴール") return [];
        return Array.from({ length: count }, () => [name, code]);
      });
  });
}

function competitionRankedRows(rows, valueIndex, rankLimit = 10) {
  ...
  return rows
    ...
    .filter(row => row[row.length - 1] <= rankLimit);
}

function refreshPlayerGoalRankingFromMatches() {
  const map = new Map();
  matchesByDate.flatMap(day => day.matches).flatMap(goalEventsFromMatch).forEach(([name, code], index) => {
    const key = `${name}__${code}`;
    const current = map.get(key) || { name, code, goals: 0, firstIndex: index };
    current.goals += 1;
    map.set(key, current);
  });

  const rows = competitionRankedRows(
    [...map.values()]
      .sort((a, b) => b.goals - a.goals || a.firstIndex - b.firstIndex)
      .map(row => [row.name, row.code, row.goals]),
    2,
    10
  );

  const card = rankingData.players.find(item => item.title === "得点数ランキング");
  if (card && rows.length) card.rows = rows;
}
...
async function loadPublicSheetData() {
  ...
  refreshPlayerGoalRankingFromMatches();
}
...
function rankingRow(item, index, type) {
  if (type === "players") {
    return `
      <div class="ranking-row">
        <span class="rank-number">${item[3] || index + 1}</span>
  ...
  return `
    <div class="ranking-row">
      <span class="rank-number">${item[2] || index + 1}</span>
...
function renderRankingCard(card, index, type) {
  ...
  const limit = query ? filteredRows.length : expandedRankings.has(key) ? filteredRows.length : Math.min(filteredRows.length, 5);
  ...
  return `
    <article class="card ranking-card">
      ...
      ${!query && filteredRows.length > 5 ? `<button class="more-button" type="button" data-more="${key}">${expandedRankings.has(key) ? "閉じる" : "もっと見る"}</button>` : ""}
    </article>
  `;
}
