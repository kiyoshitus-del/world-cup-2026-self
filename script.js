const flags = {
  ...
  NOR: "🇳🇴", SRB: "🇷🇸", AUT: "🇦🇹", PAR: "🇵🇾", PAN: "🇵🇦", NZL: "🇳🇿", JAM: "🇯🇲", BOL: "🇧🇴",
...
const correctedScheduleMatches = [
  ...csvRows(correctedGroupCsv).map(row => {
    ...
    const resultMap = {
      "MEX-RSA": [2, 0, ["メキシコ: ラウール・ヒメネス, ほか1名確認中", "南アフリカ: -"]],
      "KOR-CZE": [2, 1, ["韓国: ファン・インボム, オ・ヒョンギュ", "チェコ: ラディスラフ・クレイチー"]],
      "CAN-BIH": [1, 1, ["カナダ: Cyle Larin", "ボスニア・ヘルツェゴビナ: Lukic"]],
      "USA-PAR": [4, 1, ["アメリカ: オウンゴール, Folarin Balogun x2, Gio Reyna", "パラグアイ: Mauricio"]]
    };
...
function sheetTableToObjects(table) {
  const rows = (table.rows || []).map(row => (row.c || []).map(normalizeSheetCell));
...
function loadPublicSheet(sheetName) {
  return new Promise((resolve, reject) => {
    ...
    script.src = `https://docs.google.com/spreadsheets/d/${PUBLIC_SHEET_ID}/gviz/tq?tqx=responseHandler:${callbackName}&sheet=${encodeURIComponent(sheetName)}`;
  ...
  const nextMatches = rows.map(row => {
    ...
    return status === "finished"
      ? {
          ...base,
          score: row.score || "-",
          scorers: row.scorers ? String(row.scorers).split(" / ").filter(Boolean) : [],
  ...
  qs("#match-list").innerHTML = sortedMatches(day.matches).map(({ match, originalIndex }) => {
    ...
    return `
      <article class="card match-card">
        ...
        <div class="teams">
          <div class="team"><span class="flag">${flag(match.home.code)}</span><span class="team-name">${match.home.name}</span></div>
          ...
          <div class="team"><span class="flag">${flag(match.away.code)}</span><span class="team-name">${match.away.name}</span></div>
        </div>
        <div class="match-meta">
          ${isFinished ? `<div class="scorers">得点者：${match.scorers.join(" / ")}</div><button class="text-link" type="button" data-highlight="${currentDateIndex}-${originalIndex}">試合ハイライトを見る</button>` : `<span class="pill">${match.broadcast}</span>`}
        </div>
      </article>
    `;
  }).join("");
...
function setActiveRound(roundKey) {
  ...
  if (activeChip) {
    qs("#round-tabs").scrollTo({ left: activeChip.offsetLeft - 16, behavior: "smooth" });
  }
  return changed;
}
...
function renderRankingCard(card, index, type) {
  ...
  const maxRows = Math.min(card.rows.length, 10);
  const limit = expandedRankings.has(key) ? maxRows : Math.min(maxRows, 5);
  const rowsHtml = card.rows.length
    ? card.rows.slice(0, limit).map((row, rowIndex) => rankingRow(row, rowIndex, type)).join("")
  ...
  return `
    <article class="card ranking-card">
      ...
      ${maxRows > 5 ? `<button class="more-button" type="button" data-more="${key}">${expandedRankings.has(key) ? "閉じる" : "もっと見る"}</button>` : ""}
    </article>
  `;
}
...
function renderRankings() {
  ...
  if (currentRankingTab === "fifa") {
    const key = "fifa-0";
    const limit = expandedRankings.has(key) ? rankingData.fifa.length : 10;
    content.innerHTML = `
      <article class="card ranking-card is-fifa">
        ...
        <div class="ranking-list">
          ${rankingData.fifa.slice(0, limit).map((row, index) => `
            <div class="ranking-row">
        ...
        <button class="more-button" type="button" data-more="${key}">${expandedRankings.has(key) ? "閉じる" : "もっと見る"}</button>
      </article>
    `;
    return;
  }
...
function bindEvents() {
  ...
  qs("#match-list").addEventListener("touchend", event => {
    ...
    if (Math.abs(diffX) < 56 || Math.abs(diffX) < Math.abs(diffY) * 1.35) return;
    changeMatchDate(diffX < 0 ? 1 : -1);
  }, { passive: true });
  ...
  document.addEventListener("click", event => {
    ...
    if (groupChip) {
      ...
      if (groupChip.dataset.group === "A") {
        ...
        qs(`#group-${groupChip.dataset.group}`).scrollIntoView({ behavior: "smooth", block: "start" });
      }
    }
    ...
    if (roundChip) {
      ...
      const target = qs(`[data-round-column="${roundChip.dataset.round}"]`);
  ...
  qs("#bracket-scroll").addEventListener("scroll", () => {
    ...
    const left = qs("#bracket-scroll").scrollLeft;
    let active = columns[0].dataset.roundColumn;
    columns.forEach(column => {
      if (Math.abs(column.offsetLeft - left - 16) < 150) active = column.dataset.roundColumn;
    });
    if (qs(`[data-round="${active}"]`) && setActiveRound(active)) {
      qs("#tournament").scrollTo({ top: 0, behavior: "smooth" });
    }
  }, { passive: true });
}
